...
public class HeatmapTextureManager : MonoBehaviour
{
    ...
    public Gradient colorGradient; // Gradient for coloring the heatmap
    public Vector2Int textureResolution = new Vector2Int(256, 256); // Resolution of the heatmap texture
    ...
    private void UpdateKDETexture(float[,] dataGrid)
    {
        ...
        // Apply Gaussian kernel to every grid point
        for (int x = 0; x < width; x++)
        {
            for (int y = 0; y < height; y++)
            {
        ...
        // Find the maximum value in the KDE grid for normalization
        float maxValue = 0f;
